...
(SPEC section 3 (3)). It is the second deliverable inside Milestone 1
(SPEC section 8): a baseline LightGBM `/forecast` endpoint with an MAE/RMSE eval
...
SPEC section 4 (component 3) and SPEC section 10 (definition of done).
...
- Add `services/inference/` (initial slice) with FastAPI `/forecast`
  ...
  TEP variables (default: at least 3 per SPEC section 10).
...
- New deps (justified): `lightgbm` (SPEC section 5), `neuralforecast` (SPEC section 5
  for PatchTST), `mlflow` (SPEC section 5), `pyarrow` (efficient feature dataset
...
- Out of scope: PINNs, fine-tuned domain models (SPEC section 9). Online
